...
import LoaderCastCrew from './CastsCrews/LoaderCastCrew';
import { settingsReco } from '../../utils/SliderSettings';
import { MovieType } from '../../types/types';

const  fetchRecommendations = ({ queryKey }: any) => {
    const id = queryKey[1];
    return axios.get(`https://api.themoviedb.org/3/movie/${id}/recommendations?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US&page=1`)
}
...
const Recommendations = ({ id }: any) => {
  ...
  return (
    <div className='mt-[45px] w-full h-full px-6 mb-8'>
                ...
                <Slider {...settingsReco} className='cursor-grab h-full w-full group'>
                    {
                        data?.data?.results.map((movie: MovieType, id: number) => (
                            <Link to={`/movie/${movie?.id}`} className='max-w-[120px] md:max-w-[140px] h-[180px] md:h-[230px] space-y-3' key={id}>
                                {movie?.poster_path ?  
                                <img src={`https://image.tmdb.org/t/p/w200${movie?.poster_path}`} alt="poster_img" className='w-full h-full object-cover object-center bg-gray-200 rounded-xl overflow-hidden font-light'  />
                                :
                                ...
                                <div className='text-[#343333] '>
                                    <h1 className='text-sm antialiased dark:text-[#F5F7FA] '>{movie?.title}</h1>
                                </div>
                            </Link>
                        ))
                    }
